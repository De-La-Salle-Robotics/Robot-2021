...
public class AutonomousManager {
    private final int TOTAL_ROUTINES = 10;

    private void selectRoutine(int val) {
        _routineVal = val;
        switch (val) {
            case 0:
                _currentRoutine = new DriveForward();
                break;
            case 1:
                _currentRoutine = new DriveForwardScore();
                break;
            case 2:
                _currentRoutine = new ScoreAndLeave();
                break;
            case 3:
                _currentRoutine = new CenterScore();
                break;
            case 4:
                _currentRoutine = new TestDriveScoreLeave();
                break;
            case 5:
                _currentRoutine = new TestCenterScore();
                break;
            case 6:
                _currentRoutine = new FiveBall();
                break;
            case 7:
    ...
    public void updateRoutines(RobotState joysticks) {
        boolean updateRoutine =
                joysticks.routine == AutonState.NextAuto
